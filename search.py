...
def search_all(dep='', sub='', code='', inst='', credit_fulfill='', level='', cr='', next_sem='', keyword='', timings='', days=[], raw_data=raw_data):
    all_courses = []
    start = time()
    filtered = raw_data
    if(timings != '' or days != []): next_sem = '1'
    if(dep != '' and dep != 'ANY'): filtered = list(filter(lambda d: (d.get('department') == dep), filtered))
    if(sub != ''): 
        sub = str.upper(sub.strip())
        if(len(sub) > 2):
            filtered = list(filter(lambda d: (d.get("subject") == sub), filtered))
        else:
            filtered = list(filter(lambda d: (d.get("subject")[-1] == sub), filtered))
    if(code != ''): filtered = list(filter(lambda d: (d.get("code") == code.strip()), filtered))
    if(credit_fulfill != '' and credit_fulfill != 'ANY'): filtered = list(filter(lambda d: (int(credit_fulfill) in list(map(int, literal_eval(d.split('\t')[0].split('|')[5])))), filtered))
